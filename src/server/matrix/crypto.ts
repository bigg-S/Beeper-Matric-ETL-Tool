...
export class CryptoManager {
  ...
  async exportKeys(options: KeyExportOptionsCustom): Promise<string | void> {
    ...
    try {
      ...
      if (!exported || !Array.isArray(exported)) {
        throw new CryptoError(
          'Failed to export keys: Invalid export format',
      ...
      if (finalOptions.password) {
        ...
        // encryption key
        const { key } = await this.deriveKey(finalOptions.password, salt, finalOptions.iterations);
  ...
  async importKeys(keyData: string, password?: string): Promise<void> {
    ...
    try {
      ...
      if (parsedData.version && parsedData.algorithm === CryptoManager.ALGORITHM) {
        ...
        const { key } = await this.deriveKey(password, new Uint8Array(salt), parsedData.iterations);

        const decryptedData = await this.decryptData(encryptedData.buffer, key, new Uint8Array(iv));
  ...
  async encryptEvent(roomId: string, eventType: string, content: any): Promise<any> {
    ...
    try {
      return await this.crypto.encryptRoomEvent(new RoomId(roomId), eventType, content);
    } catch (error) {
